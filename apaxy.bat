...
:cli-up (
    IF not exist apaxy-master (
        echo ^> Dowload apaxy repository
        curl -JOL http://github.com/oupala/apaxy/archive/master.zip
        tar -xf apaxy-master.zip
    )
    IF exist apaxy-master (
        echo ^> Build apaxy image
        docker build --rm ^
            -t apaxy:%PROJECT_NAME% ^
            .\apaxy-master\
        echo ^> Startup docker container instance
        docker rm -f docker-hfs-%PROJECT_NAME%
        docker run -di ^
            -p 8080:8080 ^
            -v %cd%\share:/usr/local/apache2/htdocs/share ^
            --name docker-hfs-%PROJECT_NAME% ^
            apaxy:%PROJECT_NAME%
        docker exec -ti docker-hfs-infra-hfs bash -c "rm example.*"
        docker exec -ti docker-hfs-infra-hfs bash -c "rm README"
    ) else (
        echo ^> Failed: apaxy repository not exist
    )
    goto end
)
